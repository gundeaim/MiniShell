...
void eval(char *cmdline) 
{
    ...
    if(builtin_cmd(argv) == 0){
        ...
        //build the set of signals we want to block
        sigset_t signal_set;
        ...
        //printf("cmd not found, signals blocked. \n");
        ...
        if(pid == 0){
            ...
            //may have to fix because I didn't restore mask I just unblocked
            sigprocmask(SIG_UNBLOCK, &signal_set, NULL);
            int error = execve(argv[cmds[0]], &argv[cmds[0]], newenviron);

            //print error message if execve didn't run
            if(error == -1){
        ...
        //parent process 
            ...
            //place child in its own group group process ID = child process id
            setpgid(pid, pid);

            if(is_bg){addjob(jobs, pid, pid, 2, cmdline);}
            else{addjob(jobs, pid, pid, 1, cmdline);}


            //may have to fix because I didn't restore mask I just unblocked
            sigprocmask(SIG_UNBLOCK, &signal_set, NULL);
            
            //if job is a forground job
            if(!is_bg){
                waitfg(pid);
            }
            else{
...
void do_bgfg(char **argv) 
{
    ...
    if(argv[1] == NULL){
        printf("%s command requires PID or %cjobidargument\n", argv[0], '%');
        return;
    }
...
void sigchld_handler(int sig) 
{
    ...
    while(pid != 0 && pid != -1){
        //if stopped change state of job to stopped
        ...
        //if term with sigint delete job
...
void sigint_handler(int sig) 
{
    ...
    //if fg exists send signal to the group
    if(fg != 0){
        kill(-fg, SIGINT);
    }
