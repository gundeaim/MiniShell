...
void eval(char *cmdline) 
{
    ...
    int pid;
    int fd1;
    FILE *fileInputPointer;
    FILE *fileOutputPointer;
    ...
    numOfCommands = parseargs(argv, cmds, stdin_redir, stdout_redir);

    if(builtin_cmd(argv) == 0){
        if(numOfCommands == 1){ //check to see if there is only 1 command
            pid = fork(); //pid will be 0 for child
            if(pid == 0){ //child process does...

                //input redirection
                int argvIndexOfInput = stdin_redir[0];
                if(argvIndexOfInput > 0){ //this means there is redirecting to do
                    fileInputPointer = fopen(argv[argvIndexOfInput], "r");
                    fd1 = fileno(fileInputPointer);
                    dup2(fd1, 0);
                    close(fd1);
                }

                //output redirection
                int argvIndexOfOutput = stdout_redir[0];
                if(argvIndexOfOutput > 0){ //this means there is redirecting to do
                    fileOutputPointer = fopen(argv[argvIndexOfOutput], "w");
                    fd1 = fileno(fileOutputPointer);
                    dup2(fd1, 1);
                    close(fd1);
                }
        ...
        if(numOfCommands >= 2){ //check to see if there is only 2 command
            for(int i = 0; i < numOfCommands; i++){
                ...
                if(pid == 0){
                    //input redirection
                    int argvIndexOfInput = stdin_redir[i];
                    if(argvIndexOfInput > 0){ //this means there is redirecting to do
                        fileInputPointer = fopen(argv[argvIndexOfInput], "r");
                        fd1 = fileno(fileInputPointer);
                        dup2(fd1, 0);
                    ...
                    //output redirection
                    int argvIndexOfOutput = stdout_redir[i];
                    if(argvIndexOfOutput > 0){ //this means there is redirecting to do
                        fileOutputPointer = fopen(argv[argvIndexOfOutput], "w");
                        fd1 = fileno(fileOutputPointer);
                        dup2(fd1, 1);
                    ...
                    if(i != numOfCommands - 1){
                        dup2(newp[1], 1);
                        //fprintf(stderr, "First Dup2 call: %d %d \n", newp[1], 1);
                        close(newp[0]);
                    }

                    if (i != 0){
                        dup2(oldp[0], 0);
                        //fprintf(stderr, "Second Dup2 call: %d %d", oldp[0], 0);
                        close(oldp[1]);
                    }
